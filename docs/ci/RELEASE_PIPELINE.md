...
a fail-closed Pixi lock integrity audit, an all-platform source-lock SPDX
...
| `security.yml` | Pull requests, merge groups, protected branches, schedule | Pixi lock source/integrity audit, dependency review, plus Actions, C/C++, and Python CodeQL analysis | Read plus `security-events: write` only where needed |
...
The independent lock audit still validates that every committed Conda and PyPI
archive is referenced, fetched over HTTPS from the reviewed source allowlist,
and protected by a unique SHA-256 digest. Its uploaded SPDX file describes the
multi-platform build/development lock only and must never satisfy the
per-package release SBOM gate.
