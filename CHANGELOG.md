...
- Added canonical OpenFusion GUI/CLI entry points, headless identity/version
  contract, Ed25519 full-payload authentication, deterministic extraction and
  streaming final-payload legal quarantine. A verified development tar is
  produced; production trust and clean-machine runtime closure remain pending.
...
- Verified the Windows GPU-less CI correction using one Pixi-locked
  ...
  Windows validation passes the full native GUI suite.
- The first macOS signal-disconnect correction was insufficient. Added a second
  candidate that destroys the owned MDI hierarchy before QMainWindow base
  teardown; a 64-subwindow process regression passes locally, and native arm64
  and x86_64 validation remains pending.
