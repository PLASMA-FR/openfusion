...
QUARANTINE_GUARD_RELATIVE_PATH = (
    INSTALLER_RELATIVE_ROOT / "tests" / "test_thumbnail_provider_quarantine.py"
)
...
def is_installer_text_path(relative_path):
    """Select every installer blob except this self-referential guard fixture."""

    installer_parts = installer_subtree_parts(relative_path)
    return installer_parts is not None and relative_path.as_posix().casefold() != (
        QUARANTINE_GUARD_RELATIVE_PATH.as_posix().casefold()
    )
...
class ThumbnailProviderQuarantineTest(unittest.TestCase):
    ...
    def test_installer_fragment_cannot_hide_under_tests_directory(self):
        hidden_entry = self.synthetic_entry(
            "package/WindowsInstaller/tests/production-provider.nsh"
        )

        references = forbidden_installer_text_references(
            index_blob_groups=(((hidden_entry,), b'RegDLL "$INSTDIR\\provider.bin"\n'),)
        )
    ...
    def test_security_workflow_executes_quarantine_guard_directly(self):
        workflow = (
            REPOSITORY_ROOT / ".github" / "workflows" / "security.yml"
        ).read_text(encoding="utf-8")
        self.assertIn(
            "run: python3 package/WindowsInstaller/tests/"
